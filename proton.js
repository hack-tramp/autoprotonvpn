...
(async() => {
	...
	try {
		...
		request({
			...
			if ((verbose)&&(error!=null)) console.error('error:', error); // Print the error if one occurred
	   ...
	   await pg.waitForSelector('body > div.app-root > main > main > div > div:nth-child(5) > div:nth-child(1) > div.flex-item-fluid-auto.pt1.pb1.flex-no-min-children.flex-column > button', { timeout: 0});
	   await pg.click('div.plan-card:nth-child(1) > div:nth-child(3) > button:nth-child(4)');
		...
		var pwd = cryptoRandomString({length: 13, characters: 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_!.'});
		console.log('pwd: '+pwd);
		...
		await delay(500);
    if (verbose) console.log('entered username');
    ...
    await delay(1000);
		...
		await pg.keyboard.type(pwd, {delay: 100});
		await delay(500);
    if (verbose) console.log('entered password');
    //await delay(50000);
    await pg.keyboard.press('Tab');
    if (verbose) console.log('pressed tab');
    await delay(500);
    ...
    await pg.waitForSelector('.radio')
    let element = await pg.$$('.radio')
    console.log('res length: '+element.length);
    if (element.length < 2){
     throw('email verification disabled, try again later or from another ip')
    }
		await pg.keyboard.press('Enter');
		if (verbose) console.log('pressed return');
